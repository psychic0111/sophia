...
# curl -O  http://10.252.192.3/sophia-public/sophia-b2b-demo/raw/{分支名:默认master}/doc/client.sh
...
```
# 使用说明: [windows 下面使用git bash运行]
# curl -O  http://10.252.192.3/sophia-public/sophia-b2b-demo/raw/{分支名:默认master}/doc/jenkins-client.sh
# sh jenkins-client.sh

curl -O  http://10.252.192.3/sophia-public/sophia-b2b-demo/raw/master/doc/jenkins-client.sh
sh jenkins-client.sh
```
## 项目git分支管理说明
```
...
```
sophia-b2b-demo
    -- sophia-b2b-demo-api  #api端第三方调用提供request，response类库 （api协议层）
    -- sophia-b2b-demo-core #公共代码 （核心层）
    -- sophia-b2b-demo-dao #数据库操作 （数据层）
    -- sophia-b2b-demo-service  #业务逻辑 （服务层）
    -- sophia-b2b-demo-task #批处理任务 （任务层）
    -- sophia-b2b-demo-provider #api端接口实现（api实现层，提供者模式）
 -- doc #项目资料 （文档资料）
 -- README.md #项目文档 （说明文档）
```
...
## 公开第三方接口给其他应用调用
1. cd sophia-b2b-demo-api  #进入api
...
/home/web #线上部署目录结构说明,用户web,默认目录归属web用户
    ...
    -- demo-provider #项目demo-provider目录
        -- sophia-b2b-demo-provider-1.0-SNAPSHOT.jar #spring-boot 服务集合打包jar
